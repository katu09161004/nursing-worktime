...
記録画面の「🎤 音声で打刻」。業務を話すと即記録開始し、**ビープ2回（ピピッ）＋「○○、記録します」**と
音声で返す。「終わりました」で終了し、**ピー＋「○○、終わりました」**。手が塞がる場面の補助。
はい／いいえ確認は挟まない即時モード（誤りは「取り消して」／取消ボタンで戻す）。

3つの部品でできている。マイクは HTTPS（secure context）必須。

| 部品 | クラウド公開（VPS） | 院内オフライン（例: DL380） |
|---|---|---|
| 音声認識(STT) | ブラウザ内蔵（Web Speech API、Google/Apple） | 端末で録音→オンプレ文字起こし（faster-whisper 等）へPOST |
| 意図解釈 | さくらのAI Engine（OpenAI互換・国内DC） | ローカル LLM（Ollama・OpenAI互換） |
| 音声出力/ビープ | 端末TTS（SpeechSynthesis）＋ WebAudio（設定不要・常に有効） | 同左（端末内で完結） |

まず端末内の「読み・別名＋あいまい一致」で即判定し、迷ったときだけ意図解釈AIに投げる（速い・低コスト）。
STT/AIとも未設定なら端末内一致だけで動く＝**外部送信なし**。設定は `config_local.py` の
`STT` / `VOICE`（`config_local.py.example` 参照）。院内は外部通信不可なので STT=whisper・VOICE=ローカルLLMにする。
